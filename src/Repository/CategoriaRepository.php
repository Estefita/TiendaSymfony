...
class CategoriaRepository extends ServiceEntityRepository
{
    ...
    public function ListadoCategoria($colum, $tipo="ASC", $pos, $cant=10):array{
        $tbl = "c";
        $qb = $this->createQueryBuilder($tbl)
                    ->OrderBy($tbl.".".strtolower($colum),$tipo)
                    ->setFirstResult ( $pos )
                    ->setMaxResults ( $cant );
                    $query = $qb->getQuery();
    ...
    */
    ...
    public function Borrar(Categoria $categoria):Categoria {
        $em = $this->getEntityManager();
        $em->remove($categoria);
        $em->flush();
        return $categoria;
